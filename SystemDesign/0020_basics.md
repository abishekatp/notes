...
- First step of scaling: choosing correct DB: 
    - SQL: PostgreSQL, MySQL, Oracle, SQLite, Microsoft SQL
        ...
        - Because of that ensure ACID: Atomicity - either full transaction success or failure no intermediate state, Consistency - move the server from one valid state to another valid state, Isolation - When concurrent transactions are run it makes sure DB state is consistent and correct and there are no conflicts created and Durability - when server crashes make sure data is safe by either copying data to disk in regular interval and using other approaches like write ahead log.
...
- Load Balancing: To work with horizontal scaling we need load balancing. It will distribute across multiple server. It will manager fault tolerance.
...
- API Protocols:
    - Application Layer Protocols(Layer 7):
        ...
        - SSH: provide secure and encrypted way to remotely login and manager servers view command line terrace.
        ...
        - VoIP(Voice over IP): It is not a single protocol. It is a category of technologies that allows to make audio/video call over an internet. It uses SIP(Session Initiation Protocol) to find and establish a call. RTP(Real-time Transport Protocol) to actually carry the audio/video data. Used in Whatsapp Calls, Zoom, Skype.
...
- RESTful APIs
    ...
    - HTTP methods: GET(read), POST(create), PUT(full updated), PATCH(partial update), DELETE(delete). GET, OPTIONS and HEAD methods are considered safe and idempotent. The PATCH and POST are unsafe and not idempotent, PUT and DELETE are unsafe and idempotent.
        - Safe means it doesn’t not change servers state. Idempotent means same effect even if repeated multiple times. 
